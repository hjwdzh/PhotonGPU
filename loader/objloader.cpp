...
bool loadOBJ(
	...
	std::vector<glm::vec3> ave_normals;
...
bool loadScene(
	...
	while (is >> buffer) {
		if (strcmp(buffer, "camera") == 0) {
			...
			while (tmp != 15) {
				...
				if (strcmp(buffer, "-pos") == 0) {
					tmp += 1;
					is >> buffer;
					sscanf(buffer, "%f", &World::camera.x);
					is >> buffer;
					sscanf(buffer, "%f", &World::camera.y);
					is >> buffer;
					sscanf(buffer, "%f", &World::camera.z);
				} else
				if (strcmp(buffer, "-up") == 0) {
					tmp += 2;
					is >> buffer;
					sscanf(buffer, "%f", &World::camera_up.x);
					is >> buffer;
					sscanf(buffer, "%f", &World::camera_up.y);
					is >> buffer;
					sscanf(buffer, "%f", &World::camera_up.z);
				} else
				if (strcmp(buffer, "-lookat") == 0) {
					tmp += 4;
					is >> buffer;
					sscanf(buffer, "%f", &World::camera_lookat.x);
					is >> buffer;
					sscanf(buffer, "%f", &World::camera_lookat.y);
					is >> buffer;
					sscanf(buffer, "%f", &World::camera_lookat.z);
				} else
				return false;
			}
			continue;
		...
		if (strcmp(buffer, "ambient") == 0) {
			is >> buffer;
			sscanf(buffer, "%f", &World::lights.ambient.x);
			is >> buffer;
			sscanf(buffer, "%f", &World::lights.ambient.y);
			is >> buffer;
			sscanf(buffer, "%f", &World::lights.ambient.z);
			continue;
		}
		if (strcmp(buffer, "direct_light") == 0) {
			...
			while (tmp != 3) {
				...
				if (strcmp(buffer, "-dir") == 0) {
					...
					World::lights.direct_light_dir.push_back(dir);
				} else
				if (strcmp(buffer, "-color") == 0) {
					...
					World::lights.direct_light_color.push_back(color);
				} else {
					return false;
				}
			}
		...
		if (strcmp(buffer, "point_light") == 0) {
			...
			while (tmp != 3) {
				...
				if (strcmp(buffer, "-pos") == 0) {
					...
					is >> buffer;
					sscanf(buffer, "%f", &dir.z);
					World::lights.point_light_pos.push_back(dir);
				} else
				if (strcmp(buffer, "-color") == 0) {
					tmp += 2;
					glm::vec3 color;
					...
					is >> buffer;
					sscanf(buffer, "%f", &color.z);
					World::lights.point_light_color.push_back(color);
				} else {
					return false;
				}
			}
		...
		if (strcmp(buffer, "-tex") == 0) {
			is >> buffer;
			char imgbuffer[200];
			sprintf(imgbuffer,"tex\\%s", buffer);
			c->texImg = cv::imread(imgbuffer);
			continue;
		}
